...
| 6 | ~~El cambio de password nativo de `dj-rest-auth` no exige `old_password` ni invalida sesiones existentes tras el cambio — mismo problema de fondo que el #1, aplicado a esa vista puntual.~~ **ALCANCE CORREGIDO Y FASE 1 RESUELTA Y VERIFICADA EN PRODUCCIÓN (2026-08-26)** -- la vista nativa de dj-rest-auth ni está montada (no aplica); la invalidación de sesiones ya estaba resuelta desde Alto #4 (`mark_password_changed`/`revoke_all_device_sessions_for_subscriber`, re-confirmada en esta misma prueba). El gap real era que `POST /api/v1/profile/password/` no pedía la contraseña actual. Implementado en 2 fases: fase 1 (esta) agrega `oldPass` **opcional**, verificado con `verify_panaccess_credentials` + bloqueo temporal tras intentos fallidos; probados los 5 casos contra producción con una cuenta real (correcta/incorrecta/ausente/bloqueo/liberación) -- todos con el resultado esperado. Web ya lo manda. Fase 2 (pendiente, requiere confirmar Android/iOS y revisar métricas de adopción del log `DEPRECATION`) lo vuelve obligatorio. Ver `docs/VERIFICACION_CONTRASENA_ACTUAL_2026-08-26.md` y `docs/PLAN_VERIFICACION_CONTRASENA_ACTUAL_2026-08-26.md`. | `wind/api/profile/views.py`, `wind/api/profile/serializers.py`, `appConfig.py`, `wind/templates/wind/dashboard.html` | Verificado en producción; falta confirmar Android/iOS antes de pasar a obligatorio. De paso se detectó un problema aparte de cadena de certificados SSL en `backend.wind.do` (ver nota en el doc), sin relación con este hallazgo |
