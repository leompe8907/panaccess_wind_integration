...
| 2 | Secretos reales versionados en el historial de git: `SECRET_KEY`, `ENCRYPTION_KEY`, `DB_PASSWORD` en claro. Confirmado con `git log --all --full-history -- .env` — 19 commits hasta `bc6b9ff` (2026-07-01, "Dejar de versionar .env"); `git show bc6b9ff^:.env` sigue devolviendo los valores. No hay evidencia de que se hayan rotado después de sacar el archivo del tracking. **HERRAMIENTA LISTA (2026-08-25)** -- `python manage.py rotate_secrets` (`--generate`/`--apply`/`--status`), probada en este entorno sin tocar ningún valor real. Ver `docs/ROTACION_SECRETOS_COMPROMETIDOS_2026-08-25.md`. Sigue **pendiente de ejecutarse contra el servidor de producción real** (requiere acceso a Postgres real, que este entorno no tiene). | historial de git / `wind/management/commands/rotate_secrets.py` | Rotar `SECRET_KEY`/`ENCRYPTION_KEY`/`DB_PASSWORD` invalida sesiones y datos cifrados existentes — coordinar el corte antes de rotar |
...
**Los dos únicos "Urgente" no eran hallazgos nuevos** — eran el mismo problema (rutas nativas de auth) y el mismo riesgo (secretos en git) que ya estaban señalados en `REVISION_INDEPENDIENTE_2026-08-14.md`. El primero (rutas nativas) se resolvió el 2026-08-25 (ver `docs/LIMPIEZA_RUTAS_AUTH_NATIVAS_2026-08-25.md`). El segundo (secretos en el historial de git) tiene la herramienta de rotación lista y probada (`manage.py rotate_secrets`, ver `docs/ROTACION_SECRETOS_COMPROMETIDOS_2026-08-25.md`), pero la ejecución real contra producción sigue pendiente -- requiere que un operador con acceso al servidor real corra `--generate`, el `ALTER USER` en Postgres, y `--apply --db-password-already-changed`.
