...
`GET /wind/devices/` → `{"devices": [{"id","device_type","device_model","first_seen_at","last_seen_at","client_ip","country","city"}, ...]}` (el `device_token` nunca se expone acá). `country`/`city` son una ubicación **aproximada** resuelta desde `client_ip` (MaxMind GeoLite2, `wind/utils/geo_lookup.py`) -- puramente informativa para que el usuario reconozca sus propios dispositivos en la lista, nunca debe usarse para ningún control de acceso ni decisión de seguridad. Pueden venir en `null` (ambos, nunca uno solo) si la IP es privada/inválida, si no se encontró en la base, o si el backend no tiene `GEOIP_CITY_DB_PATH` configurado (ver sección 8) -- los clientes deben ocultar el dato en vez de mostrar un "—" cuando ambos sean `null`.
...
- **`country`/`city` en `GET /wind/devices/` requieren un paso de despliegue manual, todavía no hecho**: el código (`wind/utils/geo_lookup.py`) ya resuelve la IP contra una base local MaxMind GeoLite2, pero esa base (`GeoLite2-City.mmdb`) no viene en el repo -- MaxMind exige crear una cuenta gratuita y generar una license key para descargarla. Pasos pendientes en el servidor real: (1) crear cuenta en maxmind.com y generar una license key, (2) descargar `GeoLite2-City.mmdb` (vía `geoipupdate` o descarga manual) y guardarla en una ruta del servidor, (3) definir `GEOIP_CITY_DB_PATH` apuntando a esa ruta, (4) programar una actualización periódica del archivo (MaxMind lo republica cada semana). Sin estos pasos, `country`/`city` seguirán viniendo en `null` siempre -- no rompe nada, pero tampoco muestra ubicación hasta que se complete el despliegue.
