...
Actualización 2026-09-03: removida temporalmente la sección de logs de diagnóstico (faltaban destinatarios de alerta y confirmar tests contra Postgres real) -- **restaurada el mismo día** una vez cerrados esos dos puntos (20/20 tests OK, sin migraciones faltantes) y confirmado que el wiring de appVideo ya estaba implementado. Detalle en `docs/LOGS_DIAGNOSTICO_2026-09-01.md`.
...
1. La TV pide un código: `GET /wind/request-udid-manual/`, con header opcional `X-Device-Public-Key` (base64 de una clave pública RSA en PEM generada por la propia TV para este pareo -- la privada nunca sale de la TV). Rate limit: 1 cada 5 minutos por huella de dispositivo (429 `DEVICE_FP_RATE_LIMIT_EXCEEDED`), más un segundo límite independiente de 10 cada 5 minutos por IP (429 `IP_RATE_LIMIT_EXCEEDED`, agregado 2026-09-03 -- ver `docs/MITIGACION_FINGERPRINT_EVADIBLE_2026-09-03.md`). Ambos devuelven el mismo shape (`error_code`, `retry_after`, `retry_at`, `remaining_requests`); el manejo recomendado del lado cliente es el mismo para cualquiera de los dos: respetar `retry_after` y pedir un código nuevo más tarde, sin necesidad de distinguir cuál de los dos códigos llegó. Respuesta (201): `udid`, `temp_token`, `expires_at`, `expires_in_minutes` (5), `device_fingerprint`, `remaining_requests`. La TV muestra el `udid` (código corto) y guarda el `temp_token` en memoria.
...
- **Logs de diagnóstico para desarrolladores -- implementado 2026-09-01, confirmado 2026-09-03**, ver sección 7 y `docs/LOGS_DIAGNOSTICO_2026-09-01.md`. `APP_LOGS_INGEST_KEY` y `APP_LOGS_ALERT_RECIPIENTS` configurados en `.env`, `manage.py check`/`makemigrations --check`/los 20 tests de `applogs` corridos contra Postgres real sin problemas, y el wiring de appVideo (`errorReporting.js`) confirmado ya implementado.
