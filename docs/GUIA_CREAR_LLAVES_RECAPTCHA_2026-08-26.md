...
Este documento cubre la llave para **web** (score-based, tipo "Web"). Las apps móviles necesitan su propio tipo de llave (**iOS**/**Android**, con el mismo proceso pero eligiendo "Application type: iOS" o "Android" en vez de "Web", y usando el SDK nativo de reCAPTCHA en cada app en vez del script JS). Esto es lo que ya estaba señalado como "Posibles mejoras #31" en la auditoría -- pendiente de coordinar con esos equipos, fuera del alcance de este documento.
